...
def get_giga_auth(GIGA_AUTH, verbose=False, team = False):
    ...
    response = requests.request(
        ...
        headers=headers,
        data=payload,
        verify=r'..\src\russian_trusted_root_ca.cer',
    )
...
def get_giga_models(token):
    ...
    response = requests.request("GET", url, headers=headers, verify=r'..\src\russian_trusted_root_ca.cer')
...
async def giga_send_async_bulk(
    ...
    ssl_context = ssl.create_default_context(cafile=r'..\src\russian_trusted_root_ca.cer')
